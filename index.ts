...
class IRDNode {
    ...
    addNeigbhbor() {
        if (this.i < colors.length - 1) {
            this.next = new IRDNode(this.i + 1) 
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawIRDNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : IRDNode {
        var curr : IRDNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class KInvertRotDown {

    curr : IRDNode = new IRDNode(0)
    dir : number = 1 
